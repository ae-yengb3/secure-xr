  ...
  return (
    <SidebarProvider>
      <div className="min-h-screen bg-[#121212] text-white flex">
              ...
              {navItems.map((item) => (
                <SidebarMenuItem key={item.label}>
                  <SidebarMenuButton
                    ...
                    className={pathname === item.href ? "bg-[#0080ff]/20 text-[#0080ff]" : ""}
                  >
                    <Link href={item.href}>
        ...
        {/* Mobile menu */}
        <div
          className={`fixed inset-0 bg-black bg-opacity-80 z-40 md:hidden ${isMobileMenuOpen ? "block" : "hidden"}`}
          onClick={() => setIsMobileMenuOpen(false)}
        ></div>

        <div
          className={`fixed inset-y-0 left-0 w-64 bg-[#1a1a1a] z-50 transform transition-transform duration-300 ease-in-out md:hidden ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
        >
            ...
            <nav className="space-y-1 px-2">
              {navItems.map((item) => (
                <Link
                  key={item.label}
                  href={item.href}
                  className={`flex items-center space-x-3 px-3 py-2 rounded-md text-sm font-medium ${pathname === item.href ? "bg-[#0080ff]/20 text-[#0080ff]" : "text-gray-300 hover:bg-gray-800"}`}
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  <item.icon className="h-5 w-5" />
                  <span>{item.label}</span>
          ...
          <header className="bg-[#1a1a1a] border-b border-gray-800 py-3 px-4 flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-80 bg-[#212121] border-gray-700">
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    <DropdownMenuItem className="flex flex-col items-start py-2 cursor-pointer">
                      ...
                      <div className="text-sm text-gray-400">Open port detected on 192.168.1.1</div>
                      <div className="text-xs text-gray-500 mt-1">2 minutes ago</div>
                    </DropdownMenuItem>
                    <DropdownMenuItem className="flex flex-col items-start py-2 cursor-pointer">
                      <div className="font-medium">Scan Completed</div>
                      <div className="text-sm text-gray-400">Weekly scan of web server completed</div>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="flex items-center space-x-2">
                    ...
                    <span className="hidden md:inline-block text-xs text-gray-400">(Greater Accra)</span>
